...
#[test_each_connector(tags("postgres"))]
async fn all_postgres_array_id_types_work(api: &TestApi) -> TestResult {
    let pk_types = &[
        ("int2[]", "12"),
        ("int4[]", "78"),
        ("int8[]", "1293"),
    ...
    let drop_table = r#"DROP TABLE IF EXISTS "prisma-tests"."array_pk_test""#;

    for (pk_type, pk_value) in pk_types {
        let create_table = format!(
            r#"CREATE TABLE "prisma-tests"."array_pk_test" (id {}, PRIMARY KEY (id))"#,
            pk_type
        );
        api.execute_sql(drop_table).await?;
        api.execute_sql(&create_table).await?;
